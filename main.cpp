...
int main()
{
    ...
    for (int i = 0; i < arraySizes.size(); i++)
    {
        ...
        generateRandomVector(originalData, arraySizes[i]);
        ...
        out << "BUBBLE SORT, " << arraySizes[i] << ", " << bubble_sort_time << "\n";
        ...
        out << "SELECTION SORT, " << arraySizes[i] << ", " << selection_sort_time << "\n";
        ...
        out << "INSERTION SORT, " << arraySizes[i] << ", " << insertion_sort_time << "\n";

        // quick sort
        copyArray = originalData;
        start_time = clock();
        quickSortArray(0, arraySizes[i] - 1, copyArray);
        end_time = clock();
        double quick_sort_time = double(end_time - start_time) / CLOCKS_PER_SEC;
        out << "QUICK SORT, " << arraySizes[i] << ", " << quick_sort_time << "\n";
        ...
        out << "MERGE SORT, " << arraySizes[i] << ", " << merge_sort_time << "\n";
    }
...
{
    if (arr.size() <= 1)
    { // Base case: array is alreay sorted or empty.
        return;
    }
...
void bubbleSortArray(vector<int> &arr)
{
    ...
    { // outer for loop signifies iterations
...
void selectionSort(vector<int> &arr)
{
    ...
    {               // outer loop through all unsorted indicis in array
        minIdx = i; // set the minimum index value as the first element index.
        for (int k = i + 1; k < arr.size(); k++)
        { // loop through all array elements after the first element
            if (arr.at(k) < arr.at(minIdx))
            { // if an array element is less than the minimum
                // testing
                // count++;
...
void insertionSort(vector<int> &arr)
{
    for (int i = 1; i < arr.size(); i++)
    {
        ...
        while (k >= 0 && arr.at(k) > pointer)
        { // loop through all elements before extracted element that are greater
